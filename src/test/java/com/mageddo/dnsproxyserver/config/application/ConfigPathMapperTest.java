...
class ConfigPathMapperTest {
  ...
  void mustBuildConfPathRelativeToWorkDir(@TempDir Path tmpDir) {
    ...
    final var flags = ConfigFlagTemplates.defaultWithConfigPath(
        Paths.get("conf/config-custom.json"));
    ...
    // assert
    assertEquals("config-custom.json", configPath.getFileName()
        .toString()
    );
    assertEquals(workDir.getFileName()
        .toString(), configPath.getParent()
        .getParent()
        .getFileName()
        .toString()
    );

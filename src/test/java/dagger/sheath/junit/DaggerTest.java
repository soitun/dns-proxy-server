...
public @interface DaggerTest {
  ...
  /**
   * Instead of let DaggerTest create a dagger component directly using {@link #component()}, you
   * can
   * specify a customized, no args Supplier to provide the component instance, it's useful when
   * you need to run
   * some code at the tests before create the dagger component instance.
   * <p>
   * Obs: Be aware you can't access the Component Beans which will be mocked or spied otherwise
   * DaggerTest won't
  ...
  /**
   * Dagger componenent class, it can be the class annotated with `@Component` or the Dagger
  ...
  /**
   * A handler class which you can listen to intermediate events on the test and do things with

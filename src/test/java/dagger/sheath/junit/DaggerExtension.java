...
public class DaggerExtension implements Extension, BeforeAllCallback, AfterAllCallback,
  ...
  public void beforeAll(ExtensionContext context) throws Exception {
    ...
    context.getStore(DAGGER)
        .put(DAGGER_CTX, ctx);
    context.getStore(DAGGER)
        .put(DAGGER_CTX_WRAPPER, new CtxWrapper(ctx));
    context.getStore(DAGGER)
        .put(DAGGER_LIFECYCLE_HANDLER, createInstance(settings.eventsHandler()));
  ...
  public void beforeEach(ExtensionContext context) throws Exception {
    ...
    resetMocks(context);
    if (!Boolean.TRUE.equals(context.getStore(DAGGER)
        .get(DAGGER_SETUP, Boolean.class))) {
  ...
  public Object resolveParameter(ParameterContext parameterContext,
      ExtensionContext extensionContext) {
    final Class<?> pClass = parameterContext.getParameter()
        .getType();
    final var daggerCtx = extensionContext.getStore(DAGGER)
        .get(DAGGER_CTX);
  ...
  void resetMocks(ExtensionContext context) {
    final var testInstances = context.getRequiredTestInstances()
        .getAllInstances();
  ...
  void injectMocksAndSpies(ExtensionContext context) {
    ...
    for (Object testInstance : testInstances) {
      ...
      {
        final var fields = FieldUtils.getFieldsListWithAnnotation(testInstance.getClass(),
            InjectMock.class
        );
        inject(
            ctxWrapper,
  ...
  private static void inject(
    ...
    for (Field field : fields) {
      try {
        log.debug("status=injectMockSpyInField, field={} {}", field.getType()
        ...
        if (!validator.test(mock)) {
          throw new IllegalStateException(
              String.format("Mock/Stub didn't work for type: %s", field.getType()));
        }
  ...
  <T extends Annotation> Optional<T> findAnnotation(final ExtensionContext context,
      final Class<T> ann) {
  ...
  private static void writeTo(Object testInstance, Field field, Object foundInstance)
      throws IllegalAccessException {
    FieldUtils.writeField(field, testInstance, foundInstance, true);
    log.debug(
        "status=written, testClass={}, field={}, value={}, classToFind={}, generic={}",
        field.getName(), ClassUtils.getSimpleName(testInstance), foundInstance, field.getType(),
        field.getGenericType()
    );
  }

  static CtxWrapper findCtxWrapper(ExtensionContext context) {
    return context.getStore(DAGGER)
        .get(DAGGER_CTX_WRAPPER, CtxWrapper.class);
  }
  ...
  static Object createCtx(DaggerTest settings) throws Exception {
    if (settings.component() != Void.class) {
      log.debug("strategy=component, component={}, method={}", settings.component(),
          settings.createMethod()
      );
    ...
    throw new IllegalArgumentException(
        "You need to inform either: component or initializer options");
  }
  ...
  static void triggerSetupEvent(ExtensionContext context) {
    ...
    final var parent = context.getParent()
        .get();
    parent.getStore(DAGGER)
        .put(DAGGER_SETUP, true);
    log.debug(
        "status=triggeredSetupEvent, context={}, parent={}, root={}",
        context.hashCode(), parent.hashCode(), context.getRoot()
            .hashCode()
    );
  }
